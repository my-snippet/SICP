...
;; It is not original procedure. I created this.
(define exercise-db
  '(
;; from section 4.4.1
(address (Bitdiddle Ben) (Slumerville (Ridge Road) 10))
...
;; inner rule(find-son) has a sentence to query wife
;; But many person has no wife in the DB.
(rule (find-son-backup ?m ?s)
	  (and (wife ?m ?w)
		   (son ?w ?s)))
...
(rule (greats-grandson (?u . ?v) ?x ?y)
	  (or 
	   ...
	   (and (not (find-grand-son ?x ?y))		   			
			...
			(find-son ?x ?z)
			(greats-grandson ?v ?z ?y))))

(rule (greats-grandfather (?u . ?v) ?x ?y)
	  (or 
	   ;; grandfather?
